//Declaração de variáveis
var nome: texto = "Guilherme"
var ano_de_nascimento: inteiro = 2008
var salário: real = 20000
var usa_óculos:lógico = falso
var resposta: texto
var idade = (2026 - ano_de_nascimento)


resposta = leia("Você usa óculos? Responda V ou F")

se (resposta=='V'){
   usa_óculos = verdadeiro
} 

se (usa_óculos) {
    escreva('OK, você usa óculos!');
} senao {
    escreva('Que pena! Você não usa óculos!');
}

 nome = texto(leia ("digite seu nome"))
 ano_de_nascimento = inteiro(leia("Digite seu ano de nascimento"))
 salário = real(leia("digite seu salário"))
 idade = leia ("digite sua idade")


escreva ("${nome}")
escreva ("${salário}")
escreva ("${ano_de_nascimento}")
escreva (" ${idade}")
escreva (" $usa_óculos")
